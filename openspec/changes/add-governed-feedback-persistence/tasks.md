...
- [x] 2.2 [Backend / plataforma] Implementar un repositorio local bajo raíz controlada con esquema versionado, migración idempotente y rechazo de rutas externas, sin red ni base compartida. Evidencia: `app/api/services/feedback_repository.py`; `python -m py_compile app/api/services/feedback_repository.py` superado. La verificación con almacén temporal sintético corresponde a la tarea 2.3.
- [x] 2.3 [Tests / QA] Añadir pruebas sintéticas de registro conforme, clase inválida, campo sensible, fecha inválida, ruta externa e inicialización repetible. Evidencia: `python -m unittest tests.unit.test_feedback_persistence -v` superado: 6 pruebas sintéticas, 0 fallos; no usa narrativas CFPB ni identidad.
...
- [x] 3.2 [Tests / QA] Añadir pruebas sintéticas de retención, purga, agregación privada y ausencia de efectos sobre la ruta de predicción. Evidencia: `python -m unittest tests.unit.test_feedback_persistence -v` superado: 10 pruebas sintéticas, 0 fallos; cubre purga selectiva e idempotente, resumen sin UUID ni registros individuales y ausencia de escritura sin operación explícita.
...
- [x] 4.2 [Documentación] Crear evidencia agregada y actualizar documentación canónica solo si cambian realmente los estados de `MED-04`, `MED-05` o `ADV-02`; distinguir persistencia local de autenticación, servicio compartido, despliegue y MLOps. Evidencia: `reports/validation/claimvox_feedback_persistence.md`; `MED-04` y `MED-05` pasan a `En curso`, mientras `ADV-02` permanece `No iniciado`.
