...
- [x] 4.5 [Release] Tras PR fusionada y revisión humana, crear y verificar el tag anotado `v0.1.0-essential-mvp` en `dev`; documentar que es un corte local, no una release desplegada. Evidencia: PR `#44` fusionada en `dev` como `5480ec8`; tag anotado `v0.1.0-essential-mvp` publicado sobre ese commit con alcance local, revisable y no desplegado.
...
- [x] 5.2 [Coordinación] Actualizar tareas, decisiones, evidencias y Jira con enlaces reales; abrir la Pull Request hacia `dev` con riesgos y reversión. Evidencia: `PG-10` quedó en `Listo` con comentario de cierre y enlaces a la PR `#44` y al tag; las especificaciones delta se sincronizan antes del archivo formal del cambio.
