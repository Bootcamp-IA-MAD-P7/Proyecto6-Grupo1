...
- [x] 3.1 [Tests / QA] Añadir o ajustar pruebas directas para impedir el uso del test protegido, comprobar semillas, estrategia de folds y la salida agregada. Evidencia: `python -m unittest tests.unit.test_model_selection -v` superado: 8 pruebas, 0 fallos; cubre entradas prohibidas, grupo, folds, semilla, presupuesto del piloto y decisión.
- [x] 3.2 [Datos / ML] Ejecutar primero el piloto de viabilidad aprobado: 20.000 filas deterministas de `train.parquet`, 3 folds y 5 trials; generar solo evidencia agregada sin narrativas CFPB, datos brutos, binarios, credenciales ni logs sensibles. Evidencia: `reports/validation/cfpb_model_selection_pilot_xgb.md` registra macro F1 CV 0.591854 y desviación 0.009260; el piloto no verifica `MED-02` ni `MED-03`.
- [x] 3.3 [Miguel / coordinación] Revisar humanamente la recomendación o la ausencia de selección, enlazar evidencias con `PG-11` y actualizar criterios `MED-02` y `MED-03` solo si su evidencia mínima queda satisfecha. Decisión humana del 29 de julio de 2026: conservar XGBoost solo como candidato explorado; no seleccionar Champion ni modificar `MED-02` o `MED-03`, porque el piloto no aporta gap train-validación, ejecución completa ni evaluación protegida.

## 4. Cierre

- [x] 4.1 [Verificación] Ejecutar las pruebas unitarias directamente afectadas, `git diff --check` y la validación estricta del cambio antes de preparar la Pull Request. Verificación del 29 de julio de 2026: `python -m unittest tests.unit.test_model_selection -v` superado (8 pruebas, 0 fallos); `npm exec -- openspec validate govern-cfpb-model-selection --type change --strict` válido; `git diff --check` superado.
- [x] 4.2 [Documentación] Actualizar únicamente la documentación cuyo significado cambie con resultados reales, límites y decisión; no presentar ningún candidato como Champion o desplegado sin aprobación y evidencia posterior. Evidencia: `reports/validation/cfpb_model_selection_pilot_xgb.md` registra el resultado agregado, sus límites y la decisión humana de no selección.
